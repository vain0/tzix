...
module DictTest =
  ...
  let tryExecuteSuccessTest =
    test {
      let node        = theDict |> tryFirst "VainZero.Tzix.Desktop" |> Option.get
  ...
  let browseNodeTest =
    test {
      ...
      let dict'     =
        theDict
        |> addNewNode "tzix" "VainZero.Tzix.Core.UnitTest" |> fst
        |> Dict.removeNode (theDict |> tryFirst "VainZero.Tzix.Core" |> Option.get).Id
        |> Dict.incrementPriority (theDict |> tryFirst "VainZero.Tzix.Desktop" |> Option.get)
      let (dict'', subnodeIds) =
        dict' |> Dict.browseNode node.Id
      // Nodes that actually exists under the directory added.
      do! dict'' |> tryFirst "VainZero.Tzix.Core" |> Option.isSome |> assertPred
      // Nodes that actually don't exists under the directory removed.
      do! dict'' |> tryFirst "VainZero.Tzix.Core.UnitTest" |> Option.isNone |> assertPred
      // Enumerates subfiles of the directory.
      let subnodeNames =
        subnodeIds
        |> List.map (fun nodeId -> (dict'' |> Dict.findNode nodeId).Name)
      do! subnodeNames |> assertEquals ["VainZero.Tzix.Desktop"; "VainZero.Tzix.Core"]
    }
